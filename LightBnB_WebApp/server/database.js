...
const currentClause = function (queryParams) {
  let clause;
...
const getAllProperties = function (options, limit = 10) {
  ...
  if (options.owner_id) {
    ...
    queryString += `${currentClause(queryParams)} owner_id = $${queryParams.length} `;
  }
  if (options.minimum_price_per_night) {
    const parsedMinPrice = parseInt(options.minimum_price_per_night) * 100;
    queryParams.push(parsedMinPrice);
    queryString += `${currentClause(queryParams)} cost_per_night >= $${queryParams.length} `;
  }
  if (options.maximum_price_per_night) {
    const parsedMaxPrice = parseInt(options.maximum_price_per_night) * 100;
    queryParams.push(parsedMaxPrice);
    queryString += `${currentClause(queryParams)} cost_per_night <= $${queryParams.length}`;
  }
  if (options.minimum_rating) {
    const parsedMinRating = parseInt(options.minimum_rating);
    queryParams.push(parsedMinRating);
    queryString += `${currentClause(queryParams)} rating >= $${queryParams.length} `;
  }
  // 4
